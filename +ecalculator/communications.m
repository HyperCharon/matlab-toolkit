...
%   See also ecalculator.signal, ecalculator.circuit
    ...
    methods(Static)
        ...
        function info = ebno_to_snr(EbN0_dB, M, code_rate)
        %EBNO_TO_SNR Eb/N0 转 SNR
        %
        %   info = ecalculator.communications.ebno_to_snr(10, 4, 0.5)

            if nargin < 3, code_rate = 1; end
            if nargin < 2, M = 2; end

            k = log2(M);
            snr_val = EbN0_dB + 10*log10(k * code_rate);
            ...
            info.EbN0_dB = EbN0_dB;
            info.M = M;
            info.code_rate = code_rate;
            info.snr_dB = snr_val;
        end

        function info = ber_bpsk(EbN0_dB)
        %BER_BPSK BPSK 误码率
        %
        %   info = ecalculator.communications.ber_bpsk(10)
        ...
        function info = ber_qam(EbN0_dB, M)
        %BER_QAM M-QAM 误码率
        %
        %   info = ecalculator.communications.ber_qam(10, 16)
            ...
            % 近似公式
            ber = (4/k) * (1 - 1/sqrt(M)) * erfc(sqrt(3*SNR/(2*(M-1))));
        ...
        function info = link_budget(Pt_dBm, Gt_dBi, Gr_dBi, L_dB, d_m, f_Hz)
        %LINK_BUDGET 链路预算
        %
        ...
        function info = channel_capacity(SNR_dB, BW_Hz)
        %CHANNEL_CAPACITY 信道容量 (Shannon)
        %
        %   info = ecalculator.communications.channel_capacity(20, 10e6)

            SNR = 10^(SNR_dB/10);
            capacity = BW_Hz * log2(1 + SNR);
            ...
            info.capacity_Mbps = capacity / 1e6;
        end

        function info = fresnel_zone(d1_m, d2_m, f_Hz, n)
        %FRESNEL_ZONE 菲涅尔区半径
        %
        %   info = ecalculator.communications.fresnel_zone(1000, 1000, 2.4e9, 1)
            ...
            radius = sqrt(n * lambda * d1_m * d2_m / d);
        ...
        function info = doppler_shift(v_ms, f_Hz, theta)
        %DOPPLER_SHIFT 多普勒频移
        %
        %   info = ecalculator.communications.doppler_shift(30, 2.4e9, 0)

            if nargin < 3, theta = 0; end

            c = eutils.constants.c;
            shift = v_ms * f_Hz * cos(theta) / c;
