...
%   See also ecalculator.control, ecalculator.signal
    ...
    methods(Static)
        ...
        %   info = ecalculator.circuit.voltage_divider(12, 10e3, 4.7e3)
            ...
            fprintf('⚡ 分压电路计算:\n');
            fprintf('   输入电压:   %.3f V\n', Vin);
            fprintf('   R1:         %s\n', eutils.formatters.resistance(R1));
            fprintf('   R2:         %s\n', eutils.formatters.resistance(R2));
            fprintf('   输出电压:   %.4f V\n', Vout);
            fprintf('   分压比:     %.4f (%.1f%%)\n', Vout/Vin, Vout/Vin*100);
            fprintf('   电流:       %s\n', eutils.formatters.current(I));
            fprintf('   R1 功耗:    %s\n', eutils.formatters.power(P_R1));
            fprintf('   R2 功耗:    %s\n', eutils.formatters.power(P_R2));
            fprintf('   总功耗:     %s\n', eutils.formatters.power(P_total));
        ...
        %   ecalculator.circuit.rc_filter(10e3, 100e-9, 'highpass', 'plot', true)
            ...
            fprintf('🔌 RC %s 滤波器:\n', upper(type));
            fprintf('   R:          %s\n', eutils.formatters.resistance(R));
            fprintf('   C:          %s\n', eutils.formatters.capacitance(C));
            fprintf('   截止频率:   %s\n', eutils.formatters.frequency(fc));
            fprintf('   时间常数:   %s\n', eutils.formatters.time(tau));
            ...
            if opts.plot
                ...
                bode(sys, w);
                title(sprintf('RC %s Filter (fc = %s)', type, eutils.formatters.frequency(fc)));
                grid on;
            end
        ...
        %   ecalculator.circuit.rl_filter(100, 10e-3, 'lowpass')
            ...
            fprintf('🔌 RL %s 滤波器:\n', upper(type));
            fprintf('   R:          %s\n', eutils.formatters.resistance(R));
            fprintf('   L:          %s\n', eutils.formatters.inductance(L));
            fprintf('   截止频率:   %s\n', eutils.formatters.frequency(fc));
            fprintf('   时间常数:   %s\n', eutils.formatters.time(tau));
            ...
            if opts.plot
                ...
                bode(sys);
                title(sprintf('RL %s Filter (fc = %s)', type, eutils.formatters.frequency(fc)));
                grid on;
            end
        ...
        %   ecalculator.circuit.rlc_resonance(10, 10e-3, 100e-9)
            ...
            fprintf('   品质因数:   %.2f\n', Q);
            fprintf('   带宽:       %s\n', eutils.formatters.frequency(BW/(2*pi)));
        ...
        %   ecalculator.circuit.opamp_gain(100e3, 10e3, 'noninverting')  同相放大器
            ...
            switch lower(type)
                case 'inverting'
                    ...
                    fprintf('🔺 反相放大器:\n');
                    fprintf('   Rf:         %s\n', eutils.formatters.resistance(Rf));
                    fprintf('   Rin:        %s\n', eutils.formatters.resistance(Rin));
                    fprintf('   增益:       %.4f (%.2f dB)\n', gain, gain_dB);
                    fprintf('   相位:       180°\n');
